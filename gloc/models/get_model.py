...
def get_feature_model(args, model_name, cuda=True):
    if model_name.startswith('cosplace'):
        ...
        feat_model.load_state_dict(torch.load(f'/home/amax/code_render/LIB/mcloc_pure_code/data/depth_anything_v2_{args.encoder}.pth', map_location='cpu'))
    else:
        raise NotImplementedError()
...
def get_ref_model(args, cuda=True):
    ...
    feat_model = get_feature_model(args, args.feat_model, cuda)# ref_model='DenseFeatures', feat_model='DepthV2',

    if model_name == 'DenseFeatures':
        from gloc.models.refinement_model import DenseFeaturesRefiner
        model_class = DenseFeaturesRefiner
        conf = DenseFeaturesConf(clamp=args.clamp_score) # clamp_score=-1, 
